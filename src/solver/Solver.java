...
public class Solver implements OrderingAgent {

    private ProblemSpec spec = new ProblemSpec();
    private Store store;
    private List<Matrix> probabilities;

    public Solver(ProblemSpec spec) throws IOException {
        this.spec = spec;
        store = spec.getStore();
    ...
    public void doOfflineComputation() {
        ...
        while(hasTime() ){
            s = s0;
            while(s not in G) {
                a_greedy = argmax_a(isElement of A) Q(s,a)
                V(S) = Q(s, a_greedy);
                s' sampleFrom(T(s,a,s'))';
